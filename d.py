...
class DiscordThreadBot:
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler('logs/discord_bot_d.log')
            ]
        )
    ...
    def _display_startup_info(self):
        """Display startup information."""
        logging.info("🤖 Discord Thread Message Bot v2.0 (Bot D)")
        logging.info("=" * 50)
    ...
    def _display_config_help(self):
        ...
        logging.error("   • USER_TOKEN_D: Your Discord user token")
        logging.error("   • THREAD_ID_D: Your Discord thread ID")
...
def create_custom_config() -> BotConfig:
    ...
    USER_TOKEN = os.getenv('USER_TOKEN_D')
    THREAD_ID = os.getenv('THREAD_ID_D')
    
    # Validate required environment variables
    if not USER_TOKEN:
        logging.error("USER_TOKEN_D environment variable is required!")
        logging.error("Set this in GitHub Secrets for the repository.")
        sys.exit(1)
    
    if not THREAD_ID:
        logging.error("THREAD_ID_D environment variable is required!")
        logging.error("Set this in GitHub Secrets for the repository.")
        sys.exit(1)
